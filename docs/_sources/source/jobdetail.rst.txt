...
.. math::
  E_{\rm xc} \left[ \rho \right] = \int d\boldsymbol{r} \rho(\boldsymbol{r})
  \varepsilon_{\rm xc} \left[ \rho \right] (\boldsymbol{r})

.. math::
  v_{\rm xc} \left[ \rho \right](\boldsymbol{r})
  = \frac{ \delta E_{\rm xc}\left[ \rho \right] }{ \delta\rho(\boldsymbol{r}) }


libxcでは :math:`\rho` や、その勾配およびラプラシアン

.. math::
  \sigma_0(\boldsymbol{r}) = \left| \nabla \rho(\boldsymbol{r}) \right|

:math:`v_{\rm xc}` を計算するための要素としてスピン自由度を考慮した交換エネルギーの表式を考える。まず以下のように
